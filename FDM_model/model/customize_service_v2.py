...
class fatigue_driving_detection:  # 本地
    ...
    def _inference(self, data):
        ...
        self.result = {"result": {"duration": 6000, "drowsy": [], "faces": []}}
        self.temp_list = []
        ...
        self.video_writer = cv2.VideoWriter(output_video_path, fourcc, self.fps,
                                           (frame_width, frame_height))  # 注意宽度已被剪裁
        ...
        while frame is not None:
            ...
            print(f'start dealing nowFrameIdx : {self.nowFrameIdx} '
                  f'timestamp : {self.getTimestamp()} ms')
            try:
                # 画面剪裁，主驾驶位； [h, w, 通道]
    ...
    def model_1_detect(self, frame):
        ...
        if len(bbox) > 0:
            for box in bbox:
                # box[0] 是类别，box[1] 是 [center-x, center-y, width, height]
                if box[0] == 0:  # 人脸
